...
function wc_get_gallery_image_html( $attachment_id, $main_image = false ) {
	...
	return '<div class="owl-stage-outer"><div data-thumb="' . esc_url( $thumbnail_src[0] ) . '" data-thumb-alt="' . esc_attr( $alt_text ) . '" class="woocommerce-product-gallery__image"><a href="' . esc_url( $full_src[0] ) . '">' . $image . '</a></div></div>';
}
...
function wc_get_rating_html( $rating, $count = 0 ) {
	...
	if ( 0 < $rating ) {
		...
		$html  = '<div class="sam star-rating" role="img" aria-label="' . esc_attr( $label ) . '">' . wc_get_star_rating_html( $rating, $count ) . '</div>';
	}
